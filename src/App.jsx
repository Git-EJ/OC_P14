...
function App() {
  const {dispatch} = useContext(employeesDataContext);
  
  useEffect(() => {
    const ls = window.localStorage.getItem("employees")
    if (ls) {
      dispatch({ type: "SET_EMPLOYEES_DATA", payload: JSON.parse(ls) })
      return
    }
    const mockedData = generateMockedData(51)
    if (process.env.NODE_ENV === "development") {
      window.localStorage.setItem("employees", JSON.stringify(mockedData))
    }
    dispatch({ type: "SET_EMPLOYEES_DATA", payload: mockedData })
  }, [dispatch])
